...
resource "aws_security_group" "grafana_sg" {
  ...
  tags = {
    Name = "ray-serve-grafana-security-group"
  }
}
...
resource "aws_instance" "grafana" {
  ...
  root_block_device {
    ...
    tags = {
      Name = "ray-serve-grafana-root-volume"
    }
  }
...
# Download Ray dashboard files from GitHub
wget -q -O /var/lib/grafana/dashboards/default_grafana_dashboard.json https://raw.githubusercontent.com/poridhioss/MLOps/refs/heads/main/Ray-Cluster-Labs/Ray%20Serve%20Monitoring/dashboard/default_grafana_dashboard.json
wget -q -O /var/lib/grafana/dashboards/serve_grafana_dashboard.json https://raw.githubusercontent.com/poridhioss/MLOps/refs/heads/main/Ray-Cluster-Labs/Ray%20Serve%20Monitoring/dashboard/serve_grafana_dashboard.json

# Create dashboard provider configuration
cat > /etc/grafana/provisioning/dashboards/ray-dashboards.yaml <<'DASHCONFIG'
apiVersion: 1
providers:
  - name: 'Ray'
    orgId: 1
...
# Configure Grafana to allow iframe embedding
...
output "grafana_private_ip" {
  description = "Private IP of the Grafana monitoring instance"
